...
namespace Phantom
{
    public partial class PhantomMain : Form
    {
        ...
        private void Crypt()
        {
            ...
            string content = FileGen.CreateBat(_key, _iv, mode, hidden.Checked, selfDelete.Checked, runas.Checked, customCommand.Checked, Messagebox.Checked, customCMD.Text, customMessagebox.Text, fileType, rng);
        ...
        private void CheckVersion()
        {
            try
            {
                ...
                string latestversion = wc.DownloadString("https://raw.githubusercontent.com/therealcoderm/PhantomV2/main/version").Trim();
                wc.Dispose();
                if (File.Exists(AppDomain.CurrentDomain.BaseDirectory + "\\bin\\latestversion"))
                {
                    ...
                    {
                        DialogResult result = MessageBox.Show($"PhantomV2 {currentversion} is outdated. Download {latestversion}?", "Warning", MessageBoxButtons.YesNoCancel, MessageBoxIcon.Exclamation);
                        if (result == DialogResult.Yes)
                        {
                            Process.Start("https://github.com/therealcoderm/PhantomV2/releases/tag/" + latestversion);
                        }
                    }
                }
        ...
        private void UnpackSettings(SettingsObject obj)
        {
            ...
            customCommand.Checked = obj.customCommand;
            customMessagebox.Text = obj.customMessagebox;
            customCMD.Text = obj.customCMD;
            try
            {
        ...
        private SettingsObject PackSettings()
        {
            SettingsObject obj = new SettingsObject()
            {
                ...
                customCommand = customCommand.Checked,
                customMessagebox = customMessagebox.Text,
                customCMD = customCMD.Text
            };
        ...
        private void uacBypass_CheckedChanged(object sender, EventArgs e)
        {
            if (uacBypass.Checked)
            {
                // Currently UAC Bypass is a really rubbish method that is detected as hell.
                MessageBox.Show(@"UAC Bypass is Detected!", @"Warning", MessageBoxButtons.OK, MessageBoxIcon.Information);
                if (runas.Checked)
                {
                    runas.Checked = false;
                }
        ...
        private void customCommand_CheckedChanged(object sender, EventArgs e)
        {
            if (customCommand.Checked)
            {
                customCMD.Enabled = true;
            }
            else
            {
                customCMD.Enabled = false;
            }
        }
        ...
        private void Messagebox_CheckedChanged(object sender, EventArgs e)
        {
            ...
            {
                customMessagebox.Enabled = true;
            }
            else
            {
                customMessagebox.Enabled = false;
            }
        }
    }
}
